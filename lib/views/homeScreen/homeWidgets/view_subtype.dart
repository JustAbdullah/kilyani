...
/*class ViewSubType extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // زر "الكل" مع تصميم محسّن (مثال: إطار دائري مع ظل)
    Widget buildAllWidget() {
      return GetX<HomeController>(
        builder: (controller) => InkWell(
          onTap: () => controller.typeIdProduct.value = 0,
    ...
    // عنصر التظليل (shimmer) أثناء تحميل البيانات
    Widget buildShimmerItem() {
      return Shimmer.fromColors(
        baseColor: const Color.fromARGB(31, 83, 82, 82),
        highlightColor: AppColors.whiteColor,
    ...
    return Container(
      ...
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          // القائمة باستخدام GridView مع تباعد محسّن
          Obx(
            () => GridView.count(
              shrinkWrap: true,
              ...
              children: [
                buildAllWidget(),
                if (homeController.listofMainType.value.isEmpty)
                  ...List.generate(5, (index) => buildShimmerItem())
                else
                  ...homeController.listofMainType.value
                      .map(buildTypeItem)
                      .toList(),
              ],
            ),
          ...
          SizedBox(height: 7.h),
          // مؤشر بسيط مع تأثير ظل وحواف مستديرة
          Center(
            child: Container(
              width: 40.w,
              height: 6.h,
